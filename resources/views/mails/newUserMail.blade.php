    ...
    <div style="width: 100%; height: 100%; position: relative;">
        <div style="max-width: 500px; margin: auto;">
            ...
            <div style="border: solid 1px #ccc; border-radius: 5px; padding: 20px; font-family: Arial, Helvetica, sans-serif;">
                <h2>{{ __('globals.mail.welcome_register_title') }}</h2>
                <p style="line-height: 24px;">{{ __('globals.wizard.name') }}: {{ $user['name'] }}</p>
                <p style="line-height: 24px;">{{ __('globals.wizard.email') }}: {{ $user['email'] }}</p>
                <p style="line-height: 24px;">{{ __('globals.wizard.password') }}: {{ $user['password'] }}</p>
            </div>
